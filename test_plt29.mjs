...
async function test() {
  const rs = await db.execute("SELECT id, codigo, status, endereco_atual FROM paletes WHERE codigo IN ('PLT-0029')");
  console.log('Paletes:');
  console.table(rs.rows);

  const rs2 = await db.execute("SELECT c.id, c.palete_id, c.status, c.endereco, c.peso_kg FROM estoque_caixas c JOIN paletes p ON c.palete_id = p.id WHERE p.codigo IN ('PLT-0029')");
  console.log('Caixas:');
