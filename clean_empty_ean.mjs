...
async function clean() {
  await db.execute("DELETE FROM caixas_historico WHERE caixa_id IN (SELECT id FROM estoque_caixas WHERE ean_caixa = '')");
  const res = await db.execute("DELETE FROM estoque_caixas WHERE ean_caixa = ''");
  console.log(`Deletado ${res.rowsAffected} caixas com EAN vazio.`);
}
clean();
